...
import edu.umich.PowerTutor.util.TidMat;
import edu.umich.PowerTutor.util.TidMat.TidPair;
...
public class PowerEstimator implements Runnable {
  ...
  public static final int ITERATION_INTERVAL = 500; // 500 ms
...
//      deflateStream = new DeflaterOutputStream(new FileOutputStream(logFilename));
      BufferedOutputStream deflateStream = new BufferedOutputStream( new FileOutputStream(logFilename));
      ...
      */
      ...
      synchronized(fileWriteLock) {
        if(logStream != null) try {
          ...
          /** ------ @Dhinesh : NEW CODE BEGIN ------- **/ 

          //logStream.write("begin, " + iter + "\n");
          logStream.write("begin, " + SystemClock.elapsedRealtime() + ", " 
          ...
          /** ------ @Dhinesh : NEW CODE END ------- **/ 
          
          /** ------ @Dhinesh : OLD CODE BEGIN **/
          /*
          for(int i = 0; i < components; i++) {
            IterationData data = dataTemp[i];
            if(data != null) {
  ...
  public long[] getTotals(int uid, int windowType) {
    ...
    for(int i = 0; i < components; i++) {
      /* modified by Jau-Yuan Chen */
      /* ret[i] = histories.get(i).getTotal(uid, windowType) *
                 ITERATION_INTERVAL / 1000;
       */
      ret[i] = (long)(histories.get(i).getTotal(uid, windowType) *
                      ITERATION_INTERVAL / 1000.0);
    }
    return ret;
  }
  ...
  public long getRuntime(int uid, int windowType) {
    ...
    return (long)(runningTime * ITERATION_INTERVAL / 1000.0);
  }
  ...
  public UidInfo[] getUidInfo(int windowType, int ignoreMask) {
    ...
    synchronized(uidAppIds) {
      ...
      for(Integer uid : uidAppIds.keySet()) {
         ...
         */
        info.init(uid, currentPower,
            (long)(sumArray(getTotals(uid, windowType), ignoreMask) * ITERATION_INTERVAL / 1000.0),
            (long)(getRuntime(uid, windowType) * ITERATION_INTERVAL / 1000.0));
   ...
   *  Description 	: Method to compute and log battery voltage statistics 
   *  Arguments 	: None
   *  Return 		: None
   */
  public void logVoltageStats()
  {
	  
	  /* Using import edu.umich.PowerTutor.util.BatteryStats */;
	  BatteryStats bst = BatteryStats.getInstance();
	  String chargingStatus = null;
	  try{
		  /* Read the battery status file to know if it is charging or discharging. 
		   * This is not a platform independent way. Refer BatteryStats.java under util
		   * for more information about how BatteryStats works and list of files which are 
		   * read. */
		  
		  BufferedReader in = new BufferedReader(new FileReader("/sys/class/power_supply/battery/status"), 1024);
		  chargingStatus = in.readLine().trim();
	  }catch(IOException e){
		  Log.w(TAG, "Could not read battery status file");
	  }
	  writeToLog("VOLTAGE " + bst.getVoltage() + " " + chargingStatus + "\n");
  }
  
  /** Author		: Dhinesh Dharman
   *  Description 	: Method to compute and log CPU statistics 
   *  Arguments 	: None
   *  Return 		: None
   */
  public void logCPUStats()
  {
      /* added: Jau-Yuan Chen 
       * to log cpu frequency (now only consider single-core cpu)
       */
      String cur_freq=null, max_freq=null;
    
      try {
        BufferedReader in = new BufferedReader(new FileReader("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq"), 512);
        cur_freq = in.readLine().trim();
        in = new BufferedReader(new FileReader("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq"), 1024);
        max_freq = in.readLine().trim();
      } catch(IOException e) {
        Log.w(TAG, "Could not read cpu freq info");
      }
      
      writeToLog("cpu_freq, " + Integer.parseInt(cur_freq)/Float.parseFloat(max_freq) + "\n");
    
    
      PackageManager pm = context.getPackageManager();

	  /* Get a list of currently running app processes */
      ActivityManager activityManager = (ActivityManager)
    		  context.getSystemService(Context.ACTIVITY_SERVICE );
      List<ActivityManager.RunningAppProcessInfo> procInfos = activityManager.getRunningAppProcesses();

	  /* Get uid from pid and create a table for mapping pid to uid */
      Map<Integer, Integer> uidForPid = new HashMap<Integer, Integer>();
      for (int i = 0; i < procInfos.size(); ++i){
    	  uidForPid.put(procInfos.get(i).pid, 
    			  SystemInfo.getInstance().getUidForProcessInfo(procInfos.get(i)));
      }
      
      /* Get a list of currently running services */
      List<ActivityManager.RunningServiceInfo> srvInfos = activityManager.getRunningServices(Integer.MAX_VALUE);
      List<Integer> servicePidList = new ArrayList<Integer>();
      for (int i = 0; i < srvInfos.size(); ++i){
    	  servicePidList.add(srvInfos.get(i).pid);
      }
      
      long[] statsBuf;

      try{
    	  /* Read /proc/stat file to get total CPU stats */
    	  
		  //BufferedReader in = new BufferedReader (new FileReader ("/proc/stat"), 500);
          BufferedReader in = new BufferedReader(new FileReader("/proc/stat"), 512);
          String line;
		  String[] lineTokens;
		  writeToLog("cpu_total");
          while((line = in.readLine()) != null){
        	  if (line.startsWith("cpu ")){
        		  lineTokens = line.split(" ");	
        		  for (int j = 1; j < lineTokens.length; ++j){
        			  if (lineTokens[j].trim().length() > 0)
        				  writeToLog(", " + lineTokens[j].trim());
        		  }
        		  writeToLog("\n");
        	  }
          }
        	...
        	Log.w(TAG, "Could not read CPU stat file");
        }
      	
      	/* For each process, get the CPU stats */
        for (int j = 0; j < procInfos.size(); ++j){
        	int pid = procInfos.get(j).pid;
            String pidStatFile = "/proc/" + pid + "/stat";
            int uid = uidForPid.get(pid);
            String uidName = SystemInfo.getInstance().getUidName(uid, pm);
            String typeStr = "app";
            if (servicePidList.contains(pid)){
            	typeStr = "service";
            }
            	
            statsBuf = new long[15];
            TidMat TidMat = new TidMat();
            
            SystemInfo.getInstance().getPidUsrSysTime(pid, statsBuf);
            SystemInfo.getInstance().getTidUsrSysTime(pid, TidMat);
            
            writeToLog("cpu, " + uid + ", " + pid + ", "
                			 + statsBuf[SystemInfo.INDEX_USER_TIME] + ", " 
                			 + statsBuf[SystemInfo.INDEX_SYS_TIME] + ", " 
                			 + typeStr + ", " + procInfos.get(j).importance + ", " 
                			 + uidName + "\n");
            
            // Display all Tids for this Pid
            for (int t=0; t<TidMat.Tids.size(); t++) {
            	long[] tstatsBuf = TidMat.GetTInfo(t);
            	int UT = TidMat.GetUT(t);
            	int ST = TidMat.GetST(t);
            	
            	if (!(UT == 0 && ST == 0)) {
		            writeToLog("\t " + TidMat.GetTidName(t) + ", "
		            	 + TidMat.GetTid(t) + ", "
		       			 + TidMat.GetUT(t) + ", " 
		       			 + TidMat.GetST(t) + "\n");
            	}
	            
//	            Log.w(TAG, "Printing tid info: " + TidMat.GetTid(t) + ", "
//		       			 + TidMat.GetTidName(t) + " "
//	            		 + TidMat.GetUT(t) + " " 
//		       			 + TidMat.GetST(t));
        ...
        /* Create filter to accept subdirectories whose name starts with a digit */
        
         FilenameFilter filter = new FilenameFilter() {
        	 public boolean accept(File dir, String name) {
        		 return (dir.isDirectory() && java.lang.Character.isDigit(name.charAt(0)));
             }
         };
            
        /* Get the list of directories in /proc where each such dir contains info
         * about a particular process. The dirname is the PID. */
            
        File dir = new File("/proc");
        String[] children = dir.list(filter);
            
        /* For each kernel process, get the CPU stats */
            
        for (int j = 0; j < children.length; ++j){
        	int pid = 0;
            try{
            	pid = new Integer(children[j]).intValue();
            }catch (Exception e){
            	Log.w(TAG, "Could not convert " + children[j] + " to int");
            }
            /* Ignore processes already reported under running app processes */
            if (uidForPid.containsKey(pid)) continue;
            int uid = SystemInfo.getInstance().getUidForPid(pid);
            statsBuf = new long[7];
            SystemInfo.getInstance().getPidUsrSysTime(pid, statsBuf);
                
            /* Ignore processes which are not running currently (utime + stime = 0) */
            if (statsBuf[SystemInfo.INDEX_USER_TIME] + statsBuf[SystemInfo.INDEX_SYS_TIME] == 0)
            	continue;
                
            String typeStr = "kernel";
            String uidName = SystemInfo.getInstance().getUidName(uid, pm);
  ...
  /** Author		: Dhinesh Dharman
   *  Description 	: Method to compute and log display (screen) statistics 
   *  Arguments 	: None
   *  Return 		: None
   */
  public void logDisplayStats(){
	  PackageManager pm = context.getPackageManager();
	  ActivityManager activityManager = (ActivityManager)
    		  context.getSystemService(Context.ACTIVITY_SERVICE );
      ...
      PowerManager powm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
      //PowerManager.WakeLock wl = powm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "My Tag");
      //WindowManager.LayoutParams lp =  SystemInfo.getInstance().getActivity().getWindow().getAttributes();
      //display = ((WindowManager) getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().	
      
      float currBrightnessValue = 0;
      ...
      try{
        /* screen brightness: 0-255 */
		  ...
		  Log.w(TAG, "Brightness setting not found");
      }
	
      int screen_on = (powm.isScreenOn()? 1: 0);
      writeToLog("display, " + ai.uid + ", " 
      				+ screen_on + ", "
      				+ currBrightnessValue/255.0 + ", " 
      				+ SystemInfo.getInstance().getUidName(ai.uid, pm) +"\n");
  ...
  public void logNetworkStats(){
	  ...
	  /* Get uid from pid and create a Set of uids */
      Set <Integer> uidSet = new HashSet<Integer>();
      for (int i = 0; i < procInfos.size(); ++i){
    	  uidSet.add(SystemInfo.getInstance().getUidForProcessInfo(procInfos.get(i)));
      }
      
      
      /* Checking connectivity of Wifi and mobile interfaces */
      int mobileState = 0;  // 0: disconnected, 1: connected
      int wifiState = 0;    // 0: disconnected, 1: connected
      ...
      writeToLog("network_total, "
    		  	+ mobileState + ", "
    		  	+ wifiState + ", "
    		  	+ TrafficStats.getTotalRxBytes() + ", "
    		  	+ TrafficStats.getTotalTxBytes() + "\n");
      
      /* Get Tx/Rx bytes for each uid */
      for (Integer uid : uidSet){
    	  writeToLog("network, " + uid + ", " + TrafficStats.getUidRxBytes(uid.intValue()) + ", " 
    			  + TrafficStats.getUidTxBytes(uid.intValue()) + "\n");
      }
      
      writeToLog("mobile_total, "
