...
//    Log.w(TAG, "Checking lastInts");
    if(lastInts == null || lastInts.length < sz) {
//      Log.w(TAG, "Making lastInts size: " + sz);
      lastInts = new int[sz];
    ...
    for(int i = 0; i < sz; i++) {
      try {
        ...
        lastInts[pos++] = v;
//        Log.w(TAG, "manual Put into lastInts: " + v);
      } catch(NumberFormatException e) {
//    	  Log.w(TAG, "ManualGINumberFormatException on dir: " + dir + " e: " + e);
      }
    }
  ...
  private int[] manGetInts(String dir, int[] lastInts) {
	    File[] files = new File(dir).listFiles();
	    int sz = files == null ? 0 : files.length;
//	    Log.w(TAG, "Checking lastInts");
	    if(lastInts == null || lastInts.length < sz) {
//	      Log.w(TAG, "Making lastInts size: " + sz);
	    ...
	    for(int i = 0; i < sz; i++) {
	      try {
	        ...
	        int v = Integer.parseInt(files[i].getName());
	        lastInts[pos++] = v;
//	        Log.w(TAG, "man Put into lastInts: " + v);
	      } catch(NumberFormatException e) {
	    	Log.w(TAG, "manGINumberFormatException on dir: " + dir + " e: " + e);
	      }
	    }
	    while(pos < lastInts.length) lastInts[pos++] = -1;
	    return lastInts;
...
//  private idPair[] manualGetIdPairs(idPair[] lastIds) {
//	    String dir = "/proc/";
//	    File[] files = new File(dir).listFiles();
//	    int sz = files == null ? 0 : files.length;
//	    if(lastIds == null || lastIds.length < sz) {
//	    	lastIds = new idPair[sz];
//	    } else if(2 * sz < lastIds.length) {
//	    	lastIds = new idPair[sz];
//	    }
//	    int pos = 0;
//	    for(int i = 0; i < sz; i++) {
//	      try {
//	        int v = Integer.parseInt(files[i].getName());
//	        String TidLoc = "/proc/"+v+"/task/";
//	        int[] tids = manualGetInts(TidLoc, null);
//	        lastIds[pos++] = new idPair(v, tids);
//	        //lastInts[pos++] = v;
//	      } catch(NumberFormatException e) {
//	      }
//	    }
//	    int[] blanktid = {0}; 
//	    while(pos < lastIds.length) lastIds[pos++] = new idPair(-1, blanktid);
//	    return lastIds;
//	  }
...
public boolean getTidUsrSysTime(int pid, TidMat TidList) {
	    int[] Tids = manGetInts("/proc/" + pid + "/task/", null);
	    long[] times = new long[15];
	    String[] TIDName = new String[2];
//	    if(methodReadProcFile == null) return false;
	    
//	    Log.w(TAG, "Going to check Tids");
	    if (Tids.length > 0) {
//	    	Log.w(TAG, "Tid length: " + Tids.length);
		    for (int tid=0; tid<Tids.length; tid++) {
			    try {
//			    	Log.w(TAG, "tid: " + tid);
//			    	methodReadProcFile.invoke(null, "/proc/" + pid + "/task/" + Tids[tid] + "/stat", TIDName, null, times, null);
			    	methodReadProcFile.invoke(null, "/proc/" + pid + "/task/" + Tids[tid] + "/stat", PROCESS_STATS_FORMAT, TIDName, times, null);
//			    	Log.w(TAG, "Adding tid: " + Tids[tid] + " with info: " + times[SystemInfo.INDEX_USER_TIME] +" " + times[SystemInfo.INDEX_SYS_TIME]);
			    	TidList.AddTid(Tids[tid], times, TIDName[1]);
			    } catch(IllegalAccessException e) {
			      Log.w(TAG, "Failed to get pid cpu usage");
			    } catch(InvocationTargetException e) {
			      Log.w(TAG, "Exception thrown while getting pid cpu usage");
